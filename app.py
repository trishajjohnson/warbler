...
def profile():
    ...
    if form.validate_on_submit():
        ...
        pw_check = User.authenticate(username=user.username, password=form.password.data)

        if pw_check:
            user.username = form.username.data
            user.email = form.email.data
            user.image_url = form.image_url.data or DEFAULT_IMG_URL
            user.header_image_url = form.header_image_url.data or DEFAULT_HEADER_IMG_URL
...
def like_unlike_msg(msg_id):
    """Adds or removes msg id from g.user's likes"""
    if not g.user:
        flash("Access unauthorized.", "danger")
        return redirect("/")
...
def show_likes(user_id):
    """Displays all likes on a user's profile page when likes button is clicked."""
